...
#feats="apply-cmvn --norm-vars=$norm_vars --utt2spk=ark:$sdata/JOB/utt2spk scp:$sdata/JOB/cmvn.scp scp:$sdata/JOB/feats.scp ark:- |"
#$add_deltas && feats="$feats add-deltas ark:- ark:- |" 
feats="$feats splice-feats  $splice_opts  ark:-  ark:- |"

#feats="$feats python steps_sru/nnet-forward-myseq_nolap_sru_dnn.py $srcdir/dnn.nnet.pth $srcdir/dnn.priors.csv $splice_size $srcdir/learning.json  |"
feats="$feats python3 steps_sru/nnet-forward_sru.py $srcdir/dnn.nnet.pth $srcdir/dnn.priors.csv $splice_size $srcdir/learning.json  |"

#$feats
#latgen-faster-mapped --max-active=$max_active --beam=$beam --lattice-beam=$latbeam --acoustic-scale=$acwt --allow-partial=true --word-symbol-table=$graphdir/words.txt $dnndir/final.mdl $graphdir/HCLG.fst "$feats" "ark:|gzip -c > $dir/lat.1.gz "
#$feats
